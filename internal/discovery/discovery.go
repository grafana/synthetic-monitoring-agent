// Package discovery resolves sets of peer or instance addresses from
// heterogeneous sources: DNS names (expanded to every A/AAAA record, the way
// a Kubernetes headless Service resolves to all of its backing pods) and
// go-discover provider configurations (currently the k8s provider, pod
// discovery via the Kubernetes API). It is consumed by the cluster ring
// (peer join addresses) and by the browser pool (crocochrome instances).
package discovery
...
// DiscoverFn resolves the current set of addresses. Callers re-invoke it
// periodically so membership changes (scale-ups, restarts) are picked up;
// it returns the full current set on every call.
type DiscoverFn func() ([]string, error)

// NewDiscoverer builds a DiscoverFn from a list of entries. Each entry is
...
func NewDiscoverer(entries []string, logger *log.Logger) (DiscoverFn, error) {
	...
	// startup rather than on the first discovery invocation.
	for _, e := range entries {
		if isProviderConfig(e) {
			if p := parseConfig(e)["provider"]; p != "k8s" {
				return nil, fmt.Errorf("discovery: unsupported provider %q (only %q is supported)", p, "k8s")
			}
		}
	}
	...
	return func() ([]string, error) {
		...
		// (e.g. a transient DNS blip) should not discard addresses found by
		// the others, and periodic re-invocation recovers on the next call.
...
// resolveDNS resolves a host[:port] entry to addresses. A literal IP is
