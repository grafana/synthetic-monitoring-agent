...
export default function() {
	...
	match = new RegExp('session\u003D([a-z0-9]+)').exec(response.body); vars['sessionId'] = match ? match[1] || match[0] : null;
	vars['pageTitle'] = response.html('h1').html();
	vars['csrf'] = response.html('input[name\u003Dcsrf]').first().attr('value');
