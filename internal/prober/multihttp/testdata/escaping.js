...
export default function() {
	...
	console.log("Starting request to http://example.org/héllo/${tenant}...");
	try {
		url = new URL('http://example.org/héllo/'+vars['tenant']);
	} catch(e) {
		console.error("Invalid URL: http://example.org/héllo/${tenant}");
		fail()
	}
	

	body = encoding.b64decode("aMOpbGxvICR7dGVuYW50fQ", 'rawstd', "s");
	...
	response = http.request('GET', url.toString(), body, {
		...
		tags: {
		  ...
		  __raw_url__: 'http://example.org/héllo/${tenant}',
		},
		redirects: 0,
		headers: {'Content-Type':"text/plain","X-Script":'\u003C/script\u003E '+vars['tenant'],"X-Emoji":'emoji 🎉 '+vars['tenant'],"X-Hyphen":'${my-var}',"X-Quote":'it\'s a '+vars['tenant'],"X-Backslash":'back\\slash '+vars['tenant']}
	});
	console.log("Response received from http://example.org/héllo/${tenant}, status", response.status);
	if(logResponse) {
		const body = response.body || ''
		console.log("Response body received from http://example.org/héllo/${tenant}:", body.slice(0, 1000));
	}
	if(response.error) {
