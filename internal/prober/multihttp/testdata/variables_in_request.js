...
export default function() {
	...
	console.log("Starting request to http://example.org/${tenant}/items...");
	try {
		url = new URL('http://example.org/'+vars['tenant']+'/items');
	} catch(e) {
		console.error("Invalid URL: http://example.org/${tenant}/items");
		fail()
	}
	url.searchParams.append(vars['filterName'], vars['filterValue']);
	url.searchParams.append('page', '1');
	

	body = encoding.b64decode("eyJ0ZW5hbnQiOiIke3RlbmFudH0iLCJhZ2FpbiI6IiR7dGVuYW50fSIsInVzZXIiOiIke3VzZXJuYW1lfSJ9", 'rawstd', "s");
	body=body.replaceAll('${tenant}', vars['tenant']);
	body=body.replaceAll('${username}', vars['username']);
	...
	response = http.request('POST', url.toString(), body, {
		...
		tags: {
		  ...
		  __raw_url__: 'http://example.org/${tenant}/items',
		},
		redirects: 0,
		headers: {'Content-Type':"application/json","Authorization":'Bearer '+vars['accessToken'],"X-Tenant":vars['tenant']}
	});
	console.log("Response received from http://example.org/${tenant}/items, status", response.status);
	if(logResponse) {
		const body = response.body || ''
		console.log("Response body received from http://example.org/${tenant}/items:", body.slice(0, 1000));
	}
	if(response.error) {
