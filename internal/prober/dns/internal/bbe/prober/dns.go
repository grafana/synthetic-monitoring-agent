...
func validRRs(rrs *[]dns.RR, v *config.DNSRRValidator, logger logger.Logger) bool {
	...
	if len(*rrs) == 0 && len(v.FailIfNotMatchesRegexp) > 0 {
		logger.Log("level", "error", "msg", "fail_if_not_matches_regexp specified but no RRs returned")
		return false
	}
	if len(*rrs) == 0 && len(v.FailIfNoneMatchesRegexp) > 0 {
		logger.Log("level", "error", "msg", "fail_if_none_matches_regexp specified but no RRs returned")
		return false
	}
	for _, rr := range *rrs {
		logger.Log("level", "info", "msg", "Validating RR", "rr", rr)
		for _, re := range v.FailIfMatchesRegexp {
			match, err := regexp.MatchString(re, rr.String())
			if err != nil {
				logger.Log("level", "error", "msg", "Error matching regexp", "regexp", re, "err", err)
				return false
			}
			if match {
				logger.Log("level", "error", "msg", "At least one RR matched regexp", "regexp", re, "rr", rr)
				return false
			}
		}
		...
		for _, re := range v.FailIfNotMatchesRegexp {
			...
			if !match {
				logger.Log("level", "error", "msg", "At least one RR did not match regexp", "regexp", re, "rr", rr)
				return false
			}
		}
	...
	if len(v.FailIfAllMatchRegexp) > 0 && !allMatch {
		logger.Log("level", "error", "msg", "Not all RRs matched regexp")
		return false
	}
	if len(v.FailIfNoneMatchesRegexp) > 0 && !anyMatch {
		logger.Log("level", "error", "msg", "None of the RRs did matched any regexp")
		return false
	}
	return true
}
...
func validRcode(rcode int, valid []string, logger logger.Logger) bool {
	var validRcodes []int
	...
	if valid == nil {
		...
		for _, rcode := range valid {
			...
			if !ok {
				logger.Log("level", "error", "msg", "Invalid rcode", "rcode", rcode, "known_rcode", dns.RcodeToString)
				return false
			}
	...
	for _, rc := range validRcodes {
		if rcode == rc {
			logger.Log("level", "info", "msg", "Rcode is valid", "rcode", rcode, "string_rcode", dns.RcodeToString[rcode])
			return true
		}
	}
	logger.Log("level", "error", "msg", "Rcode is not one of the valid rcodes", "rcode", rcode, "string_rcode", dns.RcodeToString[rcode], "valid_rcodes", validRcodes)
	return false
}

func ProbeDNS(ctx context.Context, target string, module config.Module, registry *prometheus.Registry, logger logger.Logger) bool {
	success, _ := ProbeDNSWithError(ctx, target, module, registry, logger)
	return success
}

func ProbeDNSWithError(ctx context.Context, target string, module config.Module, registry *prometheus.Registry, logger logger.Logger) (bool, error) {
	...
	if module.DNS.QueryClass != "" {
		...
		if !ok {
			logger.Log("level", "error", "msg", "Invalid query class", "Class seen", module.DNS.QueryClass, "Existing classes", dns.ClassToString)
			return false, errors.New("Invalid query class")
		}
	}
	...
	if module.DNS.QueryType != "" {
		...
		if !ok {
			logger.Log("level", "error", "msg", "Invalid query type", "Type seen", module.DNS.QueryType, "Existing types", dns.TypeToString)
	...
	if !(module.DNS.TransportProtocol == "udp" || module.DNS.TransportProtocol == "tcp") {
		logger.Log("level", "error", "msg", "Configuration error: Expected transport protocol udp or tcp", "protocol", module.DNS.TransportProtocol)
		return false, errors.New("Invalid transport protocol")
	}
	...
	if err != nil {
		...
		return false, errors.Join(err, errors.New("Error resolving address"))
	}
	...
	if module.DNS.DNSOverTLS {
		if module.DNS.TransportProtocol == "tcp" {
			...
			logger.Log("level", "error", "msg", "Configuration error: Expected transport protocol tcp for DoT", "protocol", module.DNS.TransportProtocol)
			return false, errors.New("Invalid transport protocol")
		}
	}
	...
	if module.DNS.DNSOverTLS {
		...
		if err != nil {
			logger.Log("level", "error", "msg", "Failed to create TLS configuration", "err", err)
			return false, errors.Join(err, errors.New("Failed to create TLS configuration"))
		}
	...
	if len(module.DNS.SourceIPAddress) > 0 {
		...
		if srcIP == nil {
			logger.Log("level", "error", "msg", "Error parsing source ip address", "srcIP", module.DNS.SourceIPAddress)
			return false, errors.New("Error parsing source ip address")
		}
		logger.Log("level", "info", "msg", "Using local address", "srcIP", srcIP)
	...
	for retry := 0; retry <= module.DNS.Retries; retry++ {
		...
		logger.Log("level", "info", "msg", "Making DNS query", "target", targetIP, "dial_protocol", dialProtocol, "query", module.DNS.QueryName, "type", qt, "class", qc, "retry", retry)

		if module.DNS.RetryTimeout > 0 {
			client.Timeout = module.DNS.RetryTimeout
		} else {
		...
		if err != nil {
			logger.Log("level", "error", "msg", "Error while sending a DNS query", "err", err)

			if errors.Is(err, context.DeadlineExceeded) {
				// We ran out of time. The probe was not successful.
				return false, err
			}
			...
			return false, errors.Join(err, errors.New("Error while sending a DNS query"))
		}

		logger.Log("level", "info", "msg", "Got response", "response", response, "retry", retry)
		...
		if !validRcode(response.Rcode, module.DNS.ValidRcodes, logger) {
			return false, errors.New("Invalid rcode")
		}
		logger.Log("level", "info", "msg", "Validating Answer RRs")
		if !validRRs(&response.Answer, &module.DNS.ValidateAnswer, logger) {
			logger.Log("level", "error", "msg", "Answer RRs validation failed")
			return false, errors.New("Answer RRs validation failed")
		}
		logger.Log("level", "info", "msg", "Validating Authority RRs")
		if !validRRs(&response.Ns, &module.DNS.ValidateAuthority, logger) {
			logger.Log("level", "error", "msg", "Authority RRs validation failed")
			return false, errors.New("Authority RRs validation failed")
		}
		logger.Log("level", "info", "msg", "Validating Additional RRs")
		if !validRRs(&response.Extra, &module.DNS.ValidateAdditional, logger) {
			logger.Log("level", "error", "msg", "Additional RRs validation failed")
			return false, errors.New("Additional RRs validation failed")
		}

		// We did everything we needed to do. The probe was successful.
		return true, nil
	}

	return false, errors.New("Too many retries.")
}
