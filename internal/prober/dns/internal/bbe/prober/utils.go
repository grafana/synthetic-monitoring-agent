...
func chooseProtocol(ctx context.Context, IPProtocol string, fallbackIPProtocol bool, target string, registry *prometheus.Registry, logger logger.Logger) (ip *net.IPAddr, lookupTime float64, err error) {
	...
	logger.Log("level", "info", "msg", "Resolving target address", "target", target, "ip_protocol", IPProtocol)
	...
	if !fallbackIPProtocol {
		...
		if err == nil {
			for _, ip := range ips {
				logger.Log("level", "info", "msg", "Resolved target address", "target", target, "ip", ip.String())
				probeIPProtocolGauge.Set(protocolToGauge[IPProtocol])
				probeIPAddrHash.Set(ipHash(ip))
				return &net.IPAddr{IP: ip}, lookupTime, nil
			}
		}
		logger.Log("level", "error", "msg", "Resolution with IP protocol failed", "target", target, "ip_protocol", IPProtocol, "err", err)
		return nil, 0.0, err
	}

	ips, err := resolver.LookupIPAddr(ctx, target)
	if err != nil {
		logger.Log("level", "error", "msg", "Resolution with IP protocol failed", "target", target, "err", err)
		return nil, 0.0, err
	}
	...
	logger.Log("level", "info", "msg", "Resolved target address", "target", target, "ip", fallback.String())
