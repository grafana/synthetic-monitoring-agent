...
func (tm *Manager) updateTenant(tenant sm.Tenant) {
	...
	// if we do not have a tenant, or if the tenant we have is older
	// than the one we are trying to insert, we need to update it.
	if info.tenant == nil || info.validUntil.Before(now) || info.tenant.Modified < tenant.Modified {
		info.validUntil = getDeadline(now, tm.timeout, secretsTimeout, tenant.SecretStore != nil)
		info.tenant = &tenant
	}
...
func (tm *Manager) GetTenant(ctx context.Context, req *sm.TenantInfo) (*sm.Tenant, error) {
	...
	if err == nil {
		info.validUntil = getDeadline(time.Now(), tm.timeout, secretsTimeout, tenant.SecretStore != nil)
		info.tenant = tenant
