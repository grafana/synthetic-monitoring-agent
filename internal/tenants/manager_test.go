...
func TestWaitForTenant(t *testing.T) {
	t.Run("returns first tenant ID", func(t *testing.T) {
		...
		got, err := tm.WaitForTenant(ctx)
		require.NoError(t, err)
		require.Equal(t, int64(42), got)

		// Calling again should return immediately with the same value.
		got, err = tm.WaitForTenant(ctx)
		require.NoError(t, err)
		require.Equal(t, int64(42), got)
	})

	t.Run("returns first tenant even when multiple arrive", func(t *testing.T) {
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer cancel()
		...
		// Allow both tenants to be processed.
