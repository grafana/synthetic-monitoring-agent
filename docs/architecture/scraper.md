...
`pkg/collector/` is the public, single-execution adapter. It accepts an
API-level check, probe identity, and caller-supplied prober, then returns the
same transformed metrics and logs without scheduling or publishing them.
...
flowchart LR
    ...
    Collector[pkg/collector] -->|one execution| Scraper
    Caller -->|supplied Probe| Collector
...
External callers that need the transformation pipeline without the agent run
loop use `collector.New(...)` and `(*collector.Collector).Collect(ctx, t)`.
The supplied `collector.Probe` registers metrics and emits logs through a
small public interface; execution IDs remain owned and generated by the
scraper. Callers must close the collector when it is no longer needed to
release its internal scraper context. User labels default to the agent's
prefixed mode; callers can select a different tenant-compatible shape with
`collector.WithLabelMode(...)`.

`t` controls the emitted event timestamp only. The collector does not install
a virtual clock: its context deadline is still wall-clock time, and a supplied
probe that returns immediately will not trigger that deadline merely because
it reports a larger logical duration. Historical generators must resolve
logical timeout outcomes before calling the collector. This timestamp policy
is isolated to the unscheduled collector; the normal scheduled scraper keeps
its existing wall-clock wrapper-log behavior.
...
| `collector.New(...)` / `Collector.Collect(...)` | `pkg/collector/collector.go` | Public, unscheduled single-execution adapter. |
