...
- **The `k6`, `traceroute` and `protocol-secrets` feature flags are deprecated.** They are permanently enabled. `notifyAboutDeprecatedFeatureFlags` logs a hint if someone still passes them on the command line.
- **Protocol secrets are always advertised.** The agent reports `ProbeInfo.SupportsProtocolSecrets=true` at registration unconditionally, in both the Updater and the Adhoc handler. Advertising support does not resolve anything on its own: `${secrets.<name>}` interpolation is enabled per check by `HttpSettings.SecretManagerEnabled`, so a check that did not opt in is never affected.
